...
interface UseDialogueProps {
  ...
  setGameTime?: (gameTime: GameTime) => void; // 可选：允许AI返回的状态更新游戏时间
...
export const useDialogue = ({
  ...
  const handleAction = async (actionText: string, isSystemAction = false) => {
    ...
    if (!isSystemAction && backpackItems.length > 0) {
      ...
      const isGiftIntent = giftKeywords.some((keyword) =>
      ...
      const isUseIntent = useKeywords.some((keyword) =>
        actionLower.includes(keyword),
      );
      ...
      if (isGiftIntent || isUseIntent) {
        ...
        const matchedItem = backpackItems.find((item) => {
          ...
          if (itemWords.length > 0) {
            // 检查是否包含物品名称的关键词
            const hasKeyWord = itemWords.some((word) =>
        ...
        if (matchedItem) {
          ...
          if (isGiftIntent) {
            if (
              matchedItem.type === "clothing" &&
              onGiftClothing &&
              matchedItem.outfitId
            ) {
              // 赠送服装
              await onGiftClothing(
                matchedItem.outfitId,
                matchedItem.id,
                handleAction,
              );
    ...
    if (!isSystemAction && advance) {
      ...
      const isIndoorMove = [
        ...
        "走廊",
        "家",
      ].some((keyword) => actionLower.includes(keyword));
      const isOutdoorMove = [
        "电影院",
        "商城",
        "游乐园",
    ...
    if (todaySummary && nonSystemMessages.length > historyLimit + 3) {
      ...
      // 没有总结或消息不多，直接使用最近的消息
      history = nonSystemMessages.slice(-historyLimit).map((m) => ({
        role: m.sender === "user" ? "user" : "model",
        content: m.text,
      }));
    }
...
5. DO NOT update location in your response - user and Wenwan remain in different locations.)`;
    ...
    try {
      ...
      // 清理：如果 AI 在 reply 中包含 JSON 块（例如完整状态 JSON），尽量剥离 JSON，保留可读文本回复
      try {
        if (typeof replyText === "string") {
          // 去掉 code block 中的 ```json ... ```
          replyText = replyText.replace(/```json[\s\S]*?```/i, "").trim();

          // 如果以一个 JSON 对象开头，尝试解析并提取其中的 reply 字段
          ...
          if (trimmed.startsWith("{")) {
            try {
              ...
              if (
                ...
                replyText = obj.reply.trim();
              } else {
                // 若无法解析出 reply，尝试移除首个 JSON 对象（find first closing brace sequence）
                const m = trimmed.match(/\}[\s\r\n]+/);
                if (m && typeof m.index === "number") {
                  replyText = trimmed.slice(m.index + m[0].length).trim();
                }
              }
            } catch (e) {
              // 解析失败：尝试移除首个 JSON 块（`}` 后的文本作为回复）
              const m = trimmed.match(/\}[\s\r\n]+/);
              if (m && typeof m.index === "number") {
                replyText = trimmed.slice(m.index + m[0].length).trim();
              }
        ...
        console.warn("[useDialogue] 清理 reply 时出错:", e);
      }
      const isThirdPersonDescription =
        replyText.includes("温婉") ||
      ...
      // 添加到消息列表：只在主界面显示经过清理后的游戏文本（replyText）
      const displayInMain = Boolean(
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          // metadata: 当 AI 在 reply 中包含仅用于内部的内容（如完整状态 JSON、思维链），
          // characterService.js 已尽量把这些移到 response.status 或 innerThought。
          displayInMain,
          rawFull: response.raw || undefined,
        },
      ]);
      ...
      // 如果AI返回了 gameTime（某些生成器会把时间写入 status），应用到全局游戏时间
      try {
        if (
          (response as any).status &&
          (response as any).status.gameTime &&
          setGameTime
        ) {
          setGameTime((response as any).status.gameTime as GameTime);
        }
      } catch (e) {
        // 忽略
      }
      ...
      if (response.status.location && setUserLocation && !isRemoteWeChat) {
        ...
        if (detectedLocation && response.status.location === detectedLocation) {
          if (userLocation !== detectedLocation) {
            console.log(
              `[useDialogue] 自动更新用户位置: ${userLocation} → ${detectedLocation} (根据对话内容)`,
            );
            setUserLocation(detectedLocation);

            // 如果位置变化，根据移动类型智能推进时间
            if (advance) {
              const isIndoorLocation = [
                "master_bedroom",
                "guest_bedroom",
                "living_room",
                "dining_room",
              ...
              if (isIndoorLocation) {
                ...
                advance(minutes);
                console.log(
                  `[useDialogue] 家中移动检测，额外推进${minutes}分钟`,
                );
                ...
                advance(minutes);
                console.log(
                  `[useDialogue] 外出移动检测，额外推进${minutes}分钟`,
                );
              }
            }
          }
          ...
          if (userLocation !== response.status.location) {
            console.log(
              `[useDialogue] 自动更新用户位置: ${userLocation} → ${response.status.location} (对话中提到"一起"或"和")`,
            );
            ...
            if (advance) {
              const isIndoorLocation = [
                ...
                "toilet",
                "hallway",
              ].includes(response.status.location);
              const isOutdoorLocation = !isIndoorLocation;
              ...
              if (isIndoorLocation) {
                ...
                advance(minutes);
                console.log(
                  `[useDialogue] 家中移动（一起），额外推进${minutes}分钟`,
                );
              } else if (isOutdoorLocation) {
                // 外出：15-40分钟
                const nearLocations = [
                ...
                const isNearLocation = nearLocations.includes(
                ...
                advance(minutes);
                console.log(
                  `[useDialogue] 外出移动（一起），额外推进${minutes}分钟`,
                );
              }
            }
          }
          ...
          // 这种情况通常表示用户和温婉在一起
          console.log(
            `[useDialogue] 确认用户位置: ${userLocation} (与温婉位置一致)`,
          );
        }
      }
        ...
        if (lastActionRef.current) {
          ...
          setMessages((prev) => {
            const filtered = prev.filter((msg) => {
              ...
              if (
                msg.sender === "system" &&
                (msg.text.includes("AI调用失败") || msg.text.includes("❌"))
              ) {
                return false;
              }
              // 如果是重新生成，移除上一次的用户消息（避免重复）
              if (
                lastActionRef.current?.userMessageId &&
                msg.id === lastActionRef.current.userMessageId
              ) {
                return false;
              }
              return true;
