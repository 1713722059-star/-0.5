...
import { deleteSave, exportSave, getAllSaves, importSave, loadGame, saveImportedGame } from '../services/saveService';
            ...
            <div className="space-y-3">
              {canInviteSister && (
                <button
                  ...
                  className={`w-full py-3 rounded-xl font-bold text-white shadow-lg flex items-center justify-center gap-2 ${selectedFacility?.movieTypes && !selectedMovieType
              ...
              <button
                ...
                disabled={selectedFacility?.movieTypes && !selectedMovieType}
                className={`w-full py-3 rounded-xl font-bold text-white shadow-lg ${selectedFacility?.movieTypes && !selectedMovieType
                  ? 'bg-gray-400 cursor-not-allowed'
...
// 获取外观描述（完全信任AI的描述）
const getAppearanceDescription = (aiStatus?: string): string => {
  // 完全使用AI提供的描述，不做任何限制
  if (aiStatus && aiStatus.trim().length > 0) {
    return aiStatus;
  }
  // 如果AI没有提供描述，显示提示（让AI知道需要填写）
  return "（等待AI更新外观描述...）";
};
...
const BodyPartDetailModal = ({ label, part, onClose, partKey }: { label: string, part: BodyPartStatus, onClose: () => void, partKey: string }) => {
  ...
  // 完全使用AI提供的外观描述
  const appearanceDescription = getAppearanceDescription(part.status);
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
              ...
              <div className="flex flex-col gap-2">
                <div>
                  ...
                  <span className="font-bold text-gray-800 text-sm">{appearanceDescription}</span>
                </div>
