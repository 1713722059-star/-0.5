...
import { AppID, BackpackItem, GameTime } from "../types";
...
export const PhoneInterface: React.FC<PhoneInterfaceProps> = ({
    ...
    if (activeApp === AppID.HOME) {
      const gridApps = [
        ...
        },
        {
          ...
          name: "照片",
          icon: <Image size={28} />,
          bg: "bg-gradient-to-tr from-blue-400 to-purple-400",
        },
        {
          id: AppID.MAPS,
          name: "地图",
      ...
      return (
        <div
          className={`grid grid-cols-4 gap-x-3 gap-y-6 ${isMobile ? "pt-12 px-4 pb-4" : "pt-14 px-5"} min-h-full`}
        >
          {gridApps.map((app, i) => (
            <button
              key={i}
            ...
            >
              <div
                className={`w-[3.6rem] h-[3.6rem] rounded-[1.4rem] ${app.bg} shadow-xl shadow-black/20 flex items-center justify-center text-white ring-2 ring-white/20 group-active:ring-white/40 transition-all duration-200 group-hover:scale-110`}
              >
                {app.icon}
              </div>
              <span className="text-white text-[11px] font-semibold drop-shadow-lg tracking-wide group-active:opacity-70 transition-opacity">
                {app.name}
              </span>
            </button>
          ))}
  ...
  return (
    <div
      className={`${isMobile ? "w-full h-full" : "w-[380px] shrink-0 h-full"} flex items-center justify-center relative z-20 animate-fade-in delay-200`}
      style={
        isMobile
          ? ({
    ...
    >
      <div
        className={`relative ${isMobile ? "w-full h-full rounded-none" : "w-[360px] h-[780px] rounded-[3.5rem]"} bg-black shadow-[0_25px_60px_-10px_rgba(0,0,0,0.4)] ${isMobile ? "border-0" : "border-[8px] border-gray-800"} overflow-hidden ring-4 ring-gray-900/10`}
        style={
          isMobile
            ? ({
                height: "100dvh",
      ...
      >
        ...
        <div className="absolute top-2 w-full px-6 flex justify-between text-white text-[12px] font-bold z-40 drop-shadow-lg">
          {/* 动态时间和状态（优先使用传入的 gameTime / bodyStatus） */}
          <span className="text-white/95">
            {(() => {
              const gt =
                (gameTime as any) || (bodyStatus && bodyStatus.gameTime);
              if (gt && typeof gt.hour === "number") {
                const hh = String(gt.hour).padStart(2, "0");
                const mm = String(gt.minute ?? 0).padStart(2, "0");
                return `${hh}:${mm}`;
              }
              return "9:41";
            })()}
          ...
          <div className="flex gap-2 items-center">
            {bodyStatus?.temperature !== undefined && (
              <div className="flex items-center gap-1 text-sm opacity-95">
                <div className="text-white/90">{bodyStatus.temperature}°C</div>
              </div>
            )}
        ...
        <div className="w-full h-full relative overflow-y-auto overflow-x-hidden bg-gradient-to-b from-white/20 via-white/10 to-white/5 backdrop-blur-md -webkit-overflow-scrolling-touch touch-pan-y">
          <div className="min-h-full pb-4">{renderPhoneContent()}</div>
        </div>
