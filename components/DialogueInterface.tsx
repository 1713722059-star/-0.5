...
export const DialogueInterface: React.FC<DialogueInterfaceProps> = ({
  ...
  const isInSillyTavern = () => {
    try {
      return (
        window.parent !== window || (window as any).SillyTavern !== undefined
      );
    } catch (e) {
  ...
  return (
    <div
      className="flex-1 h-full flex flex-col relative z-10 animate-fade-in delay-100 min-w-0"
      style={
        {
          height: "100%",
      ...
      <div className="flex-1 border-[4px] sm:border-[6px] border-pink-200/50 rounded-[2rem] sm:rounded-[3rem] bg-gradient-to-br from-white/60 via-pink-50/40 to-purple-50/30 backdrop-blur-xl shadow-2xl shadow-pink-100/20 overflow-hidden flex flex-col relative">
        ...
        <div className="h-14 sm:h-16 flex items-center justify-between px-4 sm:px-8 border-b border-pink-200/40 bg-gradient-to-r from-pink-50/60 via-white/40 to-purple-50/40 backdrop-blur-sm">
          <div className="flex items-center min-w-0 flex-1">
            <Cloud
              className="text-pink-400 mr-2 shrink-0 drop-shadow-sm"
              size={20}
              fill="currentColor"
            />
            <span className="font-bold text-pink-600/90 tracking-widest text-sm sm:text-lg truncate drop-shadow-sm">
              我可爱的妹妹才不会这样对我
            </span>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-4 sm:p-6 space-y-4 sm:space-y-6 bg-gradient-to-b from-transparent via-pink-50/10 to-transparent">
          {messages.map((msg) => {
            // 如果消息标记为不在主界面显示，则跳过
            if ((msg as any).displayInMain === false) return null;

            // 隐藏微信消息在主界面中显示（微信消息只在手机App中显示）
            // 包括标记为isWeChat的消息和以(发送微信)/(微信)开头的消息
            if (
              msg.isWeChat ||
              msg.text.startsWith("(发送微信)") ||
              msg.text.startsWith("(微信)")
            ) {
              return null;
            }

            const displayContent = msg.text
              .replace("(发送微信)", "")
              .replace("(微信)", "");
            ...
            return (
              ...
              >
                {msg.sender === "system" ? (
                  <SystemCollapsedMessage msg={msg} isLoading={isLoading} />
                ) : (
                  <div
                    className={`relative max-w-[90%] sm:max-w-[85%] px-4 sm:px-6 py-3 sm:py-4 rounded-[1.5rem] sm:rounded-[2rem] shadow-lg backdrop-blur-md transition-all duration-300 border-2 flex flex-col ${
                      msg.sender === "user"
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <div
                        className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-wider opacity-80 ${msg.sender === "user" ? "text-blue-100" : "text-pink-400"}`}
                      >
          ...
          {/* SystemCollapsedMessage 组件定义位于文件底部 */}
...
// 可折叠的 system 消息组件（默认折叠，用户可展开查看）
const SystemCollapsedMessage: React.FC<{ msg: any; isLoading: boolean }> = ({
  msg,
  isLoading,
}) => {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-2 my-2">
      <div className="w-full text-center text-xs text-pink-400 font-medium bg-pink-50/50 py-2 px-4 rounded-full flex items-center justify-between">
        <div className="flex-1 truncate px-2">
          {open ? msg.text : preview || "（系统消息）"}
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setOpen(!open)} className="text-pink-500 px-2">
            {open ? "收起" : "显示"}
          </button>
        </div>
      </div>
      {open && msg.isRetryable && msg.retryAction && (
        <button
