...
const AppContent: React.FC = () => {
  ...
  const [tweets, setTweets] = useState<Tweet[]>([
    {
      ...
      hasImage: true,
      imageDescription:
        "照片是偷拍视角，从门缝里拍到的。哥哥正坐在书桌前认真工作，侧脸轮廓分明。阳光洒在他身上，显得格外温柔。",
      likes: 520,
      retweets: 13,
      time: "10分钟前",
      isPrivate: true,
    ...
    },
    {
      ...
      hasImage: true,
      imageDescription:
        "对着卧室的全身镜自拍。她穿着一件淡蓝色的百褶短裙，裙摆很短，露出修长白皙的双腿。她一手拿着手机挡住脸，另一只手轻轻提着裙摆，姿势有些害羞。",
      likes: 128,
      retweets: 5,
      time: "3小时前",
      isPrivate: false,
  ...
  // 用于保存编辑点的状态快照
  const messageSnapshotsRef = useRef<
    Map<
      string,
      {
        messages: Message[];
  ...
  const handleEditMessage = (messageId: string, newText: string) => {
    ...
    // 更新消息内容
    setMessages((prev) =>
      prev.map((m, i) => (i === messageIndex ? { ...m, text: newText } : m)),
    );
  };
  ...
  const handleStealUnderwear = async () => {
    ...
    if (isDiscovered) {
      // 被发现
      await handleAction(
        "(System: User sneaks into Wenwan's room at midnight to steal underwear, but Wenwan wakes up and discovers him. Generate a dramatic scene where Wenwan confronts the user. The reaction should be based on favorability: high favorability = shocked but forgiving, low favorability = angry and disappointed. Update degradation if favorability is low.)",
        true,
      );
    } else {
      // 成功偷到
      await handleAction(
        '(System: User sneaks into Wenwan\'s room at midnight and successfully steals her underwear without being discovered. Generate a scene describing the action and Wenwan sleeping peacefully. Update favorability slightly down if this is a "creepy" action, or degradation up if favorability is already low.)',
        true,
      );
    }
  ...
  const [unlockedOutfits, setUnlockedOutfits] = useState<string[]>(
    defaultUnlockedOutfits,
  );

  // User Location State
  const [userLocation, setUserLocation] =
    useState<LocationID>("master_bedroom");
  ...
  const handleBuyClothing = async (
    ...
    if (isWenwanNearby) {
      // 温婉在身边，生成剧情对话
      await handleAction(
        `(System: 哥哥在奢侈品店购买了【${name}】，温婉就在身边看到了。根据当前好感度，生成温婉的反应和对话。她可能会询问、评论、或者表现出好奇/害羞等情绪。如果好感度高，她可能会期待哥哥送给她；如果好感度低，她可能会觉得奇怪或保持距离。)`,
        true,
      );
    } else {
      // 温婉不在身边，简单描述即可
  ...
  const handleUseItem = async (
    ...
    if (isWenwanNearby && handleActionCallback) {
      ...
      await handleActionCallback(
        `(System: 哥哥使用了【${name}】（${description}），温婉就在身边。根据当前好感度和物品类型，生成温婉的反应和对话。她可能会害羞、好奇、或者表现出不同的情绪。如果好感度高，她可能会配合或接受；如果好感度低，她可能会觉得尴尬或拒绝。记得更新情绪、好感度、性欲等相关状态。)`,
        true,
      );
  ...
  const handleGiftItem = async (
    ...
    if (isWenwanNearby && handleActionCallback) {
      // 温婉在身边，生成赠送物品的剧情对话
      await handleActionCallback(
        `(System: 哥哥将【${name}】（${description}）赠送给了温婉。根据当前好感度和物品类型，生成温婉收到礼物后的反应和对话。她可能会害羞、好奇、或者表现出不同的情绪。如果好感度高，她可能会接受并配合使用；如果好感度低，她可能会觉得尴尬或拒绝。记得更新情绪和好感度。)`,
        true,
      );
      // 从背包中移除物品
  ...
  const handleGiftClothing = async (
    outfitId: string,
    itemId: string,
    handleActionCallback?: (text: string, isSystem?: boolean) => Promise<void>,
  ) => {
    ...
    if (isWenwanNearby && handleActionCallback) {
      // 温婉在身边，生成剧情对话
      await handleActionCallback(
        `(System: 哥哥将【${itemToGift.name}】赠送给了温婉。根据当前好感度，生成温婉收到礼物后的反应和对话。她可能会开心、害羞、感动等。如果好感度高，她可能会主动拥抱或亲吻；如果好感度低，她可能会礼貌地接受但保持距离。记得更新情绪和好感度。)`,
        true,
      );
    } else if (!isWenwanNearby) {
      // 温婉不在身边，简单描述即可
  ...
  const handleUseItemWithAction = async (
    itemId: string,
    name: string,
    description: string,
  ) => {
  ...
  const handleGiftItemWithAction = async (
    itemId: string,
    name: string,
    description: string,
  ) => {
  ...
  const handleGiftClothingWithAction = async (
    outfitId: string,
    itemId: string,
  ) => {
  ...
  useEffect(() => {
    const characterMessageCount = messages.filter(
    ...
    if (
      ...
      characterMessageCount % 5 === 0 &&
      characterMessageCount > lastSummaryMessageCount.current
    ) {
  ...
  return (
    <div
      className="relative w-screen h-screen overflow-hidden text-slate-800 font-sans flex items-center justify-center"
      style={
        {
    ...
    >
      ...
      {settings.displayMode === "desktop" ? (
        /* 电脑模式：3栏布局 */
        <div
          className={`flex w-full h-full max-w-[1800px] gap-6 p-6 transition-opacity duration-1000 ${gameStarted ? "opacity-100" : "opacity-0"}`}
        >
          ...
          <PhoneInterface
            ...
            onBuyItem={async (
              ...
              if (isWenwanNearby) {
                // 温婉在身边，生成剧情对话
                await handleAction(
                  `(System: 哥哥在情趣用品店购买了【${name}】，温婉就在身边看到了。根据当前好感度，生成温婉的反应和对话。她可能会害羞、好奇、或者表现出不同的情绪。如果好感度高，她可能会脸红但接受；如果好感度低，她可能会觉得尴尬或保持距离。)`,
                  true,
                );
              } else {
                // 温婉不在身边，简单描述即可
        ...
        /* 手机模式：优化的侧边栏抽屉式设计 */
        <div
          className={`relative w-full h-full overflow-hidden transition-opacity duration-1000 ${gameStarted ? "opacity-100" : "opacity-0"}`}
          style={
            {
        ...
        >
          ...
          <div className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
            ...
            <button
              ...
              className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center text-2xl sm:text-3xl shadow-xl transition-all duration-300 active:scale-95 touch-manipulation backdrop-blur-md ${
                activeApp === "tachie"
            ...
            <button
              onClick={() => {
                ...
                const isPhoneOpen =
                  activeApp !== null && activeApp !== "tachie";
                if (isPhoneOpen) {
                  setActiveApp(null);
                } else {
          ...
          {/* --- 遮罩层（侧边栏打开时显示） --- */}
          <div
            className={`absolute inset-0 bg-black/40 backdrop-blur-sm z-35 transition-opacity duration-300 ${
              activeApp !== null
          ...
          {/* --- 立绘侧边栏（从左侧滑入，优化动画和样式） --- */}
          <div
            className={`absolute left-0 top-0 bottom-0 w-[85vw] sm:w-[420px] max-w-[90vw] z-40 bg-gradient-to-br from-white via-pink-50/30 to-purple-50/20 backdrop-blur-2xl shadow-2xl transition-transform duration-300 ease-out border-r-2 border-pink-200/50 ${
              activeApp === "tachie" ? "translate-x-0" : "-translate-x-full"
            }`}
          >
          ...
          {/* --- 手机侧边栏（从左侧滑入，优化动画和样式） --- */}
          <div
            className={`absolute left-0 top-0 bottom-0 w-[100vw] sm:w-[420px] max-w-[100vw] z-40 bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/20 backdrop-blur-2xl shadow-2xl transition-transform duration-300 ease-out border-r-2 border-blue-200/50 ${
              activeApp !== null && activeApp !== "tachie"
                ? "translate-x-0"
                : "-translate-x-full"
            }`}
          >
            <div className="h-full w-full overflow-hidden flex flex-col touch-pan-y">
              ...
              <div className="flex-1 overflow-y-auto overflow-x-hidden -webkit-overflow-scrolling-touch scrollbar-thin scrollbar-thumb-blue-300 scrollbar-track-transparent">
                <PhoneInterface
                  activeApp={
                    activeApp === AppID.HOME ? AppID.HOME : (activeApp as AppID)
                  }
