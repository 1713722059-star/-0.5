    ...
    const messageHandler = (event: MessageEvent) => {
      ...
      if (event.data && event.data.id === messageId) {
        ...
        if (event.data.error) {
          ...
          const result =
            event.data.data !== undefined ? event.data.data : event.data;
          resolve(result as T);
        }
        return;
      }
...
function parseAIResponse(aiResponse: string): any {
  ...
  // 预先提取常见的标签内容，优先处理 <game> 和思维链/内心标签
  // 这样可以把 <game> 中的游戏文本作为主要 reply，且把内心/思考内容保存到额外字段，避免显示到主对话区
  const extracted: any = {};
  try {
    const gameMatch = jsonText.match(/<game>([\s\S]*?)<\/game>/i);
    if (gameMatch) {
      extracted.game = gameMatch[1].trim();
      // 从原文本中移除该块，避免后续干扰解析
      jsonText = jsonText.replace(gameMatch[0], "").trim();
    }

    // 常见思维链/内心标签（多种可能的标签名）
    const thoughtRegex =
      /<(thought|os|meow|think_nya~)>([\s\S]*?)<\/(?:thought|os|meow|think_nya~)>/gi;
    let thoughtMatches = [] as RegExpMatchArray[];
    let m: RegExpExecArray | null;
    while ((m = thoughtRegex.exec(aiResponse)) !== null) {
      if (m[2]) thoughtMatches.push(m as RegExpMatchArray);
    }
    if (thoughtMatches.length > 0) {
      extracted.innerThought = thoughtMatches
        .map((t) => t[2].trim())
        .join("\n\n");
      // 从原文本中移除这些标签
      jsonText = jsonText.replace(thoughtRegex, "").trim();
    }
  } catch (e) {
    // 不阻塞解析，继续后续处理
    console.warn("[characterService] 提取 <game>/思维标签时出错:", e);
  }
  ...
  try {
    ...
    // 如果在最开始提取到 game 或 innerThought，把它们保存在返回对象中，供上层使用
    if (extracted.game && !parsed.reply) parsed.reply = extracted.game;
    if (extracted.innerThought) {
      // 如果 status 存在，将内心写入 status.innerThought，否则放在根字段 innerThought
      if (!parsed.status) parsed.status = {};
      if (!parsed.status.innerThought)
        parsed.status.innerThought = extracted.innerThought;
      else parsed.innerThought = extracted.innerThought;
    }
    return parsed;
  } catch (secondError) {
    // 尝试修复不完整的JSON
    ...
    fixedJson = fixedJson.replace(
      /("usageCount"|"status"|"clothing"|"lastUsedBy"|"usageProcess"|"level")\s*:\s*$/m,
      "",
    );
    ...
    try {
      ...
      throw new Error(
        `JSON解析失败: ${finalError}. 原始响应: ${aiResponse.substring(0, 500)}`,
      );
    }
  }
}
...
- **40-69 (亲密接触阶段)**:
  ...
  - **重要：关于"舔脚"行为**：
    ...
    * 如果哥哥表现过于直接、粗暴或强迫，温婉会感到羞耻和不安，可能拒绝或接受但感到"下头"，这时会减少好感度（-1到-2点），不增加堕落度。
...
- **堕落度增长规则**:
  ...
  - **重要**：一次事件可以涉及多个身体部位的使用。例如：一次完整的性爱事件可能同时使用嘴（口交）、胸（抚摸）、小穴（性交）等多个部位。
  - **使用次数增长规则**：在一次事件中，如果某个身体部位被持续使用（如做爱时一直玩胸部、持续口交等），usageCount应该根据使用时长和强度增长5-15次（随机或根据实际使用情况）。例如：一次完整的做爱事件，如果持续玩弄胸部，chest的usageCount应该增长5-15次，而不是只增长1次。
  - 因此，即使堕落度只有50（约12-25次事件），如果每次事件平均使用3-4个部位且每个部位使用次数增长5-15次，开发度也可能达到lv2（150次使用）甚至lv3（350次使用）。
  - 最高一次涨2-4点，不能超过100

- **堕落度对行为的影响**:
  - 堕落度越高，温婉对黄毛/间男的抵抗越弱
  - 堕落度越高，温婉对哥哥的态度也会发生显著变化：
    * **低堕落度（0-30）**：对哥哥温柔、依赖，保持正常的兄妹/恋人关系
    * **中低堕落度（31-50）**：对哥哥开始有S倾向，会调戏哥哥，偶尔会主动挑逗，但整体还是以依赖为主
    * **中高堕落度（51-70）**：对哥哥越来越S，主动调戏并伴有轻微羞辱、控制，会开始用一些贬低的词汇（如"笨蛋哥哥"、"废物"），但还保留一定底线
    * **高堕落度（71-90）**：性格变成雌小鬼，即便哥哥的好感度高于90，温婉也有权拒绝哥哥的一切活动（如邀请做爱、外出游玩等）。她会变得任性、傲慢，会用各种方式拒绝和羞辱哥哥，享受掌控感
    * **极恶堕落度（91-100）**：温婉可以坦白给哥哥带了绿帽子，在哥哥面前彻底化身S，羞辱、掌控不在话下。她会主动告诉哥哥自己和其他男人的关系，享受哥哥的痛苦和绝望。在黄毛跟前则是纯粹的母狗，完全服从，形成强烈反差

- **堕落度与好感度的关系**:
  - 如果哥哥做出很下头的行为（强迫、不尊重等）或让温婉生气，会减少好感度（-1到-2点），不增加堕落度
  - 堕落度高时，温婉对哥哥的态度会变得更加S和掌控，但这是基于她自身的堕落，而不是因为哥哥的行为
...
- **CRITICAL: Body Part Development (身体部位开发度)**:
  ...
  * **使用次数增长规则**：
    * 在一次事件中，如果某个身体部位被持续使用（如做爱时一直玩胸部、持续口交、长时间抚摸等），usageCount应该根据使用时长和强度增长5-15次（随机或根据实际使用情况）。
    * 例如：一次完整的做爱事件，如果持续玩弄胸部，chest的usageCount应该增长5-15次，而不是只增长1次。
    * 如果只是短暂触碰，可以增长1-3次；如果是持续使用，应该增长5-15次。
  ...
  * **CRITICAL: Body Part Status Field (外观描述) - AI MUST UPDATE**:
    * The 'status' field in each body part is **REQUIRED** and should describe the CURRENT APPEARANCE and SENSATION based on the current dialogue/interaction.
    * **You MUST update the 'status' field** for body parts that are involved in the current interaction, describing their current state based on what just happened.
    * The description should be dynamic and reflect the current situation (e.g., after being touched: "微微红肿的唇瓣，还残留着刚才的触感" / after being used: "湿润饱满的双唇，已经习惯了被使用").
    * **Update rules**:
      * If a body part is used/touched/played with in the current interaction, you MUST update its 'status' field with a detailed description.
      * The description should reflect the current physical state, appearance, and sensation based on what just happened.
      * Be specific and descriptive - don't just use generic terms like "未开发". Describe what it looks/feels like NOW.
    * **Examples of dynamic status descriptions**:
      * After being kissed: mouth: "微微红肿的唇瓣，还残留着刚才的触感，唇色比平时更深一些"
      * After being touched: chest: "敏感的胸部，刚才的抚摸让皮肤泛起了淡淡的红晕"
      * After being used: groin: "湿润的私处，刚才的使用让它变得敏感，还残留着湿润感"
      * During arousal: nipples: "坚挺的乳头，因为兴奋而变得敏感，一碰就有强烈反应"
    * **IMPORTANT**: Always update 'status' based on the current dialogue context, not just based on usageCount or level.
...
You MUST respond in valid JSON format with the following structure:
{
  ...
  "status": {
    ...
    "currentAction": "正在做什么",
    "innerThought": "内心想法",
    "mouth": { "level": 0, "usageCount": 0, "status": "", "clothing": "润唇膏", "lastUsedBy": "无", "usageProcess": "暂无记录" },
    "chest": { "level": 0, "usageCount": 0, "status": "", "clothing": "真空", "lastUsedBy": "无", "usageProcess": "暂无记录" },
    "nipples": { "level": 0, "usageCount": 0, "status": "", "clothing": "乳贴", "lastUsedBy": "无", "usageProcess": "暂无记录" },
    "groin": { "level": 0, "usageCount": 0, "status": "", "clothing": "纯棉白色内裤", "lastUsedBy": "无", "usageProcess": "暂无记录" },
    "posterior": { "level": 0, "usageCount": 0, "status": "", "clothing": "无", "lastUsedBy": "无", "usageProcess": "暂无记录" },
    "feet": { "level": 0, "usageCount": 0, "status": "", "clothing": "赤足", "lastUsedBy": "无", "usageProcess": "暂无记录" },
...
function generateDynamicStatusUpdate(
  ...
  // 检查位置变化
  if (
    currentStatus.location !== lastStatus.location ||
  ...
  if (currentStatus.arcLight !== lastStatus.arcLight) {
    updates.push(
      `arcLight: ${currentStatus.arcLight ? `"${currentStatus.arcLight}"` : "null"}`,
    );
  }
  ...
  for (const part of bodyParts) {
    ...
    if (
      current.usageCount !== last.usageCount ||
      current.level !== last.level
    ) {
      bodyPartUpdates.push(
        `${part}: {level: ${current.level}, usageCount: ${current.usageCount}}`,
      );
    }
  }
  ...
  // 检查记忆更新（只发送新的记忆）
  if (
    memoryData?.todaySummary &&
    memoryData.todaySummary !== lastStatus.innerThought
  ) {
    const recentEvents = memoryData.calendarEvents?.slice(0, 3) || [];
    if (recentEvents.length > 0) {
      updates.push(
        `todaySummary: "${memoryData.todaySummary.substring(0, 100)}..."`,
      );
      updates.push(
        `recentEvents: [${recentEvents.map((e: any) => `"${e.time} ${e.title}"`).join(", ")}]`,
      );
    }
  }
 ...
 */
function filterWorldbookByArcLight(
  worldbookContent: string,
  currentArcLight: string | null,
): string {
  if (!currentArcLight) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 保留黄毛系统（如果当前是弧光B或C）
    if (
      (currentArcLight === "B" || currentArcLight === "C") &&
...
async function getSystemInstruction(
  ...
  const arcLightChanged =
    currentArcLight !== undefined && currentArcLight !== lastArcLight;
  ...
  try {
    ...
    if (source === "api" && worldbooks.length > 0) {
      ...
      worldbooks.forEach((wb, index) => {
        ...
        if (entriesText) {
          // 手机端限制世界书长度
          const processedText = limitTextLength(
            entriesText,
            MAX_WORLDBOOK_LENGTH,
            isMobile,
          );
  ...
  if (currentArcLight !== undefined) {
    ...
    const worldbookMatch = finalInstruction.match(
      /【关系演变：五大角色弧光系统】[\s\S]*?(?=\*\*CRITICAL:|$)/,
    );
    if (worldbookMatch) {
      const originalWorldbook = worldbookMatch[0];
      const filteredWorldbook = filterWorldbookByArcLight(
        originalWorldbook,
        currentArcLight,
      );
      ...
      console.log(
        `[characterService] 按弧光阶段过滤世界书：当前弧光=${currentArcLight || "试探期"}`,
      );
    } else {
      ...
      if (hasSillyTavernWorldbook) {
        console.log(
          `[characterService] 从SillyTavern获取世界书，暂不过滤（格式可能不同）`,
        );
      }
    }
  }
  ...
  if (presetContent && presetContent.trim()) {
    const processedPreset = limitTextLength(
      presetContent,
      MAX_PRESET_LENGTH,
      isMobile,
    );
...
function buildGeminiResponseFromAIText(
  ...
  try {
    ...
    console.warn(
      "[characterService] JSON解析失败，尝试备用解析方法:",
      parseError,
    );
    console.log(
      "[characterService] AI原始响应:",
    ...
    if (replyMatch && replyMatch[1]) {
      ...
      if (replyText.length > 0) {
        ...
        console.log(
          "[characterService] 使用备用解析方法成功提取reply:",
      ...
      if (
        ...
        console.log(
          "[characterService] 响应不是JSON格式，直接使用为reply:",
  ...
  let normalizedEmotion =
    parsedResponse.status?.emotion || currentStatus.emotion;
  if (!validEmotions.includes(normalizedEmotion)) {
    ...
    normalizedEmotion =
      emotionMap[normalizedEmotion] || currentStatus.emotion || "neutral";
  }

  const parsedStatus =
    parsedResponse &&
    typeof parsedResponse.status === "object" &&
    parsedResponse.status
      ? parsedResponse.status
      : null;

  // 如果 parseAIResponse 提取了内心字段（或根级 innerThought），确保它最终进入 status.innerThought
  if (parsedResponse) {
    // 优先 root-level innerThought -> status
    if (
      parsedResponse.innerThought &&
      (!parsedResponse.status || !parsedResponse.status.innerThought)
    ) {
      parsedResponse.status = parsedResponse.status || {};
      parsedResponse.status.innerThought = parsedResponse.innerThought;
    }
    // 兼容：如果 AI 单独返回了 game 文本并把其放在 parsedResponse.reply，保留不变
  }
  ...
  const mergedStatus: BodyStatus = {
    ...
    posterior: {
      ...currentStatus.posterior,
      ...(parsedStatus?.posterior || {}),
    },
...
export async function generateCharacterResponse(
  ...
  const forceSillyTavernGenerate =
    isSillyTavern && options?.useSillyTavernGenerate === true;
  ...
  async function detectSTAPI(): Promise<boolean> {
    ...
    const checkSTAPI = (win: Window): boolean => {
      try {
        if (
          typeof (win as any).ST_API !== "undefined" &&
      ...
      try {
        if (currentWindow.parent && currentWindow.parent !== currentWindow) {
          if (checkSTAPI(currentWindow.parent)) {
            console.log(
              `[characterService] 在 parent 第 ${i + 1} 层检测到 ST_API`,
            );
            return true;
          }
        ...
        console.log(
          `[characterService] 无法访问 parent 第 ${i + 1} 层（跨域限制）`,
        );
        break;
      }
    }
    ...
    if (isSillyTavern) {
      ...
      try {
        await new Promise<void>((resolve) => {
          ...
          // 检查 APP_READY 是否已经设置
          if (
            typeof (window as any).APP_READY !== "undefined" &&
            (window as any).APP_READY
          ) {
  ...
  if (typeof window !== "undefined") {
    ...
    if (
      typeof (window as any).ST_API !== "undefined" &&
        ...
        for (let i = 0; i < 5; i++) {
          try {
            if (
              ...
              if (
                typeof (currentWindow.parent as any).ST_API !== "undefined" &&
                typeof (currentWindow.parent as any).ST_API.prompt?.generate ===
                  "function"
              ) {
                canUseSTAPI = true;
                console.log(
                  `[characterService] 在 parent 第 ${i + 1} 层同步检测到 ST_API`,
                );
                break;
              }
              currentWindow = currentWindow.parent;
    ...
    if (!canUseSTAPI) {
      if (
        typeof window.TavernHelper !== "undefined" &&
          ...
          for (let i = 0; i < 5; i++) {
            try {
              if (
                ...
                if (
                  typeof (currentWindow.parent as any).TavernHelper !==
                    "undefined" &&
                  typeof (currentWindow.parent as any).TavernHelper.generate ===
                    "function"
                ) {
                  canUseTavernHelper = true;
  ...
  const preferSillyTavernAPI =
    isSillyTavern && !hasValidAPIConfig && (canUseSTAPI || canUseTavernHelper);
...
**重要**：请根据以上记忆综合分析哥哥的行为模式，判断他是否"很下头"。如果记忆显示哥哥经常做下头的事，即使当前行为轻微，也要考虑累积效应，适当减少好感度。
`
    : ""
}
...
3. **MEMORY-BASED JUDGMENT**: ${
        memoryData
          ? "根据上面的记忆数据，综合分析哥哥的行为。如果判断他很下头或让温婉生气，减少好感度（1-2点），不增加堕落度。"
          : "根据当前对话和游戏状态，判断哥哥是否很下头或让温婉生气。如果是，减少好感度（1-2点），不增加堕落度。"
      }
...
`;
  ...
  if (forceSillyTavernGenerate) {
    try {
      ...
      // 复用现有解析与状态合并逻辑
      return buildGeminiResponseFromAIText(
        stText,
        currentStatus,
        isRemoteWeChat,
      );
      ...
      console.warn(
        "[characterService] 强制使用酒馆 Generate 失败，将尝试降级到自定义接口:",
        error,
      );
  ...
  if (
    ...
    if (arcLightChanged) {
      console.log(
        `[characterService] 弧光变化（${lastArcLight} -> ${currentStatus.arcLight}），重新生成系统提示词`,
      );
  ...
  const totalPromptLength = messages.reduce(
    (sum, msg) => sum + estimatePromptTokens(msg.content),
    0,
  );
  ...
  if (totalPromptLength > 10000) {
    console.warn(
      `[characterService] Prompt过长（估算${totalPromptLength} tokens），可能导致模型无法生成回复`,
    );
  }
  ...
  try {
    ...
    if (useSillyTavernAPI || preferSillyTavernAPI) {
      ...
      console.log(
        "[characterService] 尝试使用 SillyTavern API，isSillyTavern=",
        isSillyTavern,
        "canUseSTAPI=",
        canUseSTAPI,
        "canUseTavernHelper=",
      ...
      if (stApiDetected) {
        try {
          ...
          if (stApi && stApi.prompt?.generate) {
            console.log(
              "[characterService] 成功获取 ST_API 实例，准备调用 generate",
            );
            ...
            if (result && result.text) {
              ...
              success = true;
              console.log(
                "[characterService] 使用 ST_API.prompt.generate 成功",
              );
            }
          } else {
            // 如果无法直接访问 ST_API（跨域限制），尝试通过 postMessage 代理调用
            console.log(
              "[characterService] 无法直接访问 ST_API，尝试通过 postMessage 代理调用",
            );
            try {
              const proxyResult = await requestSTAPIViaPostMessage<{
                text?: string;
              }>(
                "prompt.generate",
                {
              ...
              if (proxyResult && proxyResult.text) {
                ...
                success = true;
                console.log(
                  "[characterService] 通过 postMessage 代理调用 ST_API.prompt.generate 成功",
                );
              }
            } catch (proxyError: any) {
              console.warn(
                "[characterService] postMessage 代理调用失败:",
                proxyError,
              );
            }
          }
          ...
          console.warn(
            "[characterService] ST_API.prompt.generate 调用失败，降级到备用方法:",
            stApiError,
          );
        ...
        console.log(
          "[characterService] ST_API 检测失败，尝试通过 postMessage 代理调用",
        );
        try {
          const proxyResult = await requestSTAPIViaPostMessage<{
            text?: string;
      ...
      if (!success && canUseTavernHelper && tavernHelper) {
        try {
          ...
          console.warn(
            "[characterService] TavernHelper.generate 调用失败:",
            tavernHelperError,
          );
      ...
      if (!success || !response) {
        ...
        if (hasValidAPIConfig) {
          console.log(
            "[characterService] SillyTavern API 不可用，降级到使用配置的 API",
          );
    ...
    if (!response) {
      ...
      const estimatedMaxTokens = Math.min(
        32000,
        Math.max(baseMaxTokens, baseMaxTokens + promptBonus),
      );
      ...
      // 打印请求摘要信息
      console.log(
        `[characterService] 请求参数: model=${requestBody.model}, max_tokens=${estimatedMaxTokens}, 估算prompt_tokens=${totalPromptLength}`,
      );
      console.log(
        `[characterService] 请求体摘要: messages数量=${messages.length}, system长度=${systemInstruction.length}`,
      );
    ...
    if (
      ...
      console.error(
        "[characterService] 响应结构异常，完整响应:",
        JSON.stringify(data, null, 2),
      );
      throw new Error(
        `AI返回的响应格式不正确: ${JSON.stringify(data)}。可能是API配置问题，请检查API地址和模型设置。`,
      );
    }
    ...
    if (usage.completion_tokens === 0) {
      // 调试：打印更多信息
      console.error(
        "[characterService] 模型未生成内容，完整响应数据:",
        JSON.stringify(data, null, 2),
      );
      ...
      errorMsg +=
        " 建议：1) 检查prompt是否过长 2) 尝试减少对话历史 3) 检查模型是否支持该任务。";
    ...
    if (!aiResponse || aiResponse.trim().length === 0) {
      console.error(
        "[characterService] AI响应为空，完整响应数据:",
        JSON.stringify(data, null, 2),
      );
      ...
      errorMsg +=
        " 请检查：1) 模型是否正确 2) max_tokens是否足够（当前4000） 3) API服务是否正常。";
